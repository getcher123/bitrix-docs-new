...
**Примечание**: обязательность полей будет проверена в методах [CIBlock::Add](add.md)и [CIBlock::Update](update.md), а значение по умолчанию будет установлено только в форме редактирования элемента в административной части сайта.
...
| arFields | Массив вида array("код поля" => "значение" ...), где значение это массив содержащий следующие элементы: - IS_REQUIRED - признак обязательности заполнения (Y\|N). - DEFAULT_VALUE - значение поля по умолчанию. |
...
- [Поля элемента](../../fields.md#fiblockfields)
