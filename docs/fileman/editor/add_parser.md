...
Функция-обработчик (собственно сам парсер) должна быть объявлена в области видимости вызова метода addContentParser().
...
всем подключенным парсерам. Если парсер возвращает неизмененный фрагмент или **false**(для проверки используется условие строгого соответствия), то фрагмент передается на обработку следующему парсеру.
...
Функция-обработчик (собственно сам парсер) должна быть объявлена в области видимости вызова метода oBXEditorUtils.addPHPParser().
Объявленной функции передается единственный строковый аргумент - код документа.
В теле функции происходит обработка кода и возврат измененного значения. Если возвращаемое значение равно **false**, изменения не вносятся.
...
Функция-обработчик должна быть объявлена в области видимости вызова метода oBXEditorUtils.DOMHandler().
