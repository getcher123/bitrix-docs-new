...
- [Режим вывода ошибок (error_reporting)](#error_reporting)
...
| Описание: | Если данный параметр установлен (равен "Y"), то к телу письма снизу будет добавлено: ``` MID: EVENT_ID.TEMPLATE_ID (DATE_EVENT) ``` Где: - *EVENT_ID* - ID почтового события - *TEMPLATE_ID* - ID почтового шаблона - *DATE_EVENT* - дата создания почтового события |
| По умолчанию: | N |
...
| Описание: | Любой подкаталог каталога DocumentRoot в котором будут храниться все файлы загружаемые на сервер и сохраняемые функцией CFile::SaveFile. Данный параметр широко используется в функциях класса [CFile](reference/cfile/index.md). Под термином "загруженные файлы" понимаются как правило все файлы загружаемые через HTML формы, за исключением файлов которые могут быть загружены в любой каталог через меню "Структура сайта" -> "Управление" -> "Загрузить файл". |
| По умолчанию: | upload |
