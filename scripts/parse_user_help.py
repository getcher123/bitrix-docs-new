#!/usr/bin/env python3
"""
Парсер пользовательской документации для 1C-Bitrix
"""
...
START_URL = "https://dev.1c-bitrix.ru/user_help/"
OUTPUT_DIR = "/home/ubuntu/bitrix-docs-new/docs/user_help"
IMAGES_DIR = "/home/ubuntu/bitrix-docs-new/images/user_help"
...
def clean_content(soup):
    ...
    # Удаляем копирайты
    for elem in soup.find_all(string=re.compile(r'©.*Битрикс', re.I)):
        if elem.parent:
...
def parse_page(url, depth=0, max_depth=5):
    ...
    try:
        ...
        url_path = url_path.replace('/user_help/', '').strip('/')
    ...
    for root, dirs, files in os.walk(OUTPUT_DIR):
        for file in files:
            if file.endswith('.md'):
                ...
                content = re.sub(r'\[([^\]]+)\]\(https?://dev\.1c-bitrix\.ru/user_help/[^\)]*\)', r'\1', content)
                
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(content)

if __name__ == "__main__":
    print("Начало парсинга пользовательской документации...")
